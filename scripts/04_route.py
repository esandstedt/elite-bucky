...
class StarCollection:
    def __init__(self, list):
        self._list = list
        self._tree_array = [
            [star.coords.x, star.coords.y, star.coords.z] for star in list]
...
stars = [Star(x[0], x[1], Coords(x[2], x[3], x[4]), True)
...
node_start = Star(1, "Froarks GM-D d12-355",
                  Coords(-533.59375, 209.6875, 15375.09375))
...
def h(s): return s.coords.dist(node_goal.coords) / (4*jump_range)
...
while open_collection.any():
    ...
    print("%7d %4.3f %5d %5d" % (len(open_collection),
                                 f[current.id], lowest_dist_to_goal, int(dist)))
